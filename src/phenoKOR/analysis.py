...
for park in parks:
    ...
    for file in file_list:
        ...
        for j in range(0,4):
            ...
            # 줄리안 데이 : 10월 15일 - 289, 11월 17일 - 322, 그 사이 값의 기하 평균을 wEVI로 설정
            wEVI_df = data_class[(data_class.index >= 289) & (data_class.index <= 322)]
    ...
    for file in file_list:
        ...
        for j in range(0, 4):
            ...
            data_class = data[data['class'] == j]
            data_class['Datetime'] = data_class['date'].apply(lambda x: format(datetime.strptime(x, '%Y-%j'), '%Y-%m-%d'))
            ...
            data_class.index = doy_each # date 에서 변경 (뒤로 갈수록 DOY 값이 누적되어 커지므로 계산값 커지는 것을 방지
            data_sub.index = date
            ...
            for a in range(iter):
                ...
                par_init = opt_param
                mn, mx, sos, rsp, eos, rau = opt_param[0], opt_param[1], opt_param[2], opt_param[3], opt_param[4], opt_param[5]
            ...
            if j == 0:
                ...
                sos_gl.append(opt_param[2])  # SOS 값 추가
            elif j == 1:
                coniferous = pd.concat([coniferous, data_sub])
                sos_cf.append(opt_param[2])  # SOS 값 추가
            elif j == 2:
                broadleaved = pd.concat([broadleaved, data_sub])
                sos_bl.append(opt_param[2])  # SOS 값 추가
            else:
                mixed = pd.concat([mixed, data_sub])
                sos_mx.append(opt_param[2])  # SOS 값 추가
    ...
    for i, sos_list in enumerate(sos_all):
        for j, sos in enumerate(sos_list):
            sos = int(np.floor(sos))  # 값이 소수점으로 나와서 floor로 내림
            sos_list[j] = sos
        sos_all[i] = pd.DataFrame(sos_list, index=range(2003, 2022), columns=['sos'])
        if i == 0:
            sos_all[i].to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/sos/{park}_sos_grassland_last.csv')
            grassland.to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/{park}/{park}_DL_grassland_datetime_last.csv')
        elif i == 1:
            sos_all[i].to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/sos/{park}_sos_coniferous_last.csv')
            coniferous.to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/{park}/{park}_DL_coniferous_datetime_last.csv')
        elif i == 2:
            sos_all[i].to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/sos/{park}_sos_broadleaved_last.csv')
            broadleaved.to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/{park}/{park}_DL_broadleaved_datetime_last.csv')
        else:
            sos_all[i].to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/sos/{park}_sos_mixed_last.csv')
            mixed.to_csv(f'C:/Users/cdbre/Desktop/Project/data/pred/{park}/{park}_DL_mixed_datetime_last.csv')
